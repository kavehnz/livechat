<template>
    <div>
        <div class="form-container-step-title">
            <b class="block text-md">Hi {{ formStore.name }}</b>
            <span class="text-sm"> Need Help ? try live chat with our agent ! </span>
        </div>
        <div class="text-center font-extralight text-sm mb-3">Conversations</div>
        <!-- Agents List -->
        <button 
            class="agent-item" 
            type="button"
            @click="handleClick('Jacob')"
            :class="{'border-l-4 border-green-500': selectedAgent === 'Jacob'}"
        > 
            <div class="flex flex-row align-middle">
                <div class="agent-item-img me-3">
                    <img src="https://www.gravatar.com/avatar/2c7d99fe281ecd3bcd65ab915bac6dd5?s=250" class="w-12 h-12 rounded-full">
                </div>
                <div class="agent-item-info text-start flex flex-col">
                    <b class="text-md mt-1">Jacob</b>
                    <span class="font-extralight text-sm">Lorem ipsum dolor sit amet</span>
                </div>
            </div>
            <div class="agent-item-date">
                <span class="mb-2 mt-1">Today</span>
                <span>Now</span>
            </div>
        </button>
...
<style>
  ...
  .agent-item{
    @apply w-full cursor-pointer bg-gray-50 p-2 rounded-md flex flex-row justify-between items-center align-middle mb-3 hover:bg-gray-200 transition;
  }
  .agent-item-date { @apply flex flex-col items-end text-xs justify-between  font-extralight text-gray-500; }
  .error-label{
    @apply text-red-500 text-xs mb-3 block mt-2;
  }
  .necessary-label{
    @apply text-red-500 text-sm mb-3;
  }
  .submit-btn.ico{
    @apply cursor-pointer flex flex-row justify-center rounded-lg w-full text-white bg-black p-4 hover:bg-purple-950 transition;
  }
</style>